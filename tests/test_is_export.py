...
def build_fake_analysis(
    *, attention: bool = True, network: bool = True
    ...
    return AnalyzeResponse(
        position_encoding="absolute" if attention else "digit_roles",
        analysis_runtime="transformerlens" if attention else "native_pytorch",
...
def test_markdown_summary_includes_unavailable_notes() -> None:
    ...
    assert "03000000 + 03000000 = <ans>" in summary
...
def test_cli_missing_checkpoint_fails_clearly(
    ...
    with pytest.raises(SystemExit):
        cli.main(["--prompt", "03000000 + 03000000 = <ans>", "--output", str(output)])
