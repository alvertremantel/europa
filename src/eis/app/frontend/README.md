...
Then run the frontend from `src/eis/app/frontend/`:
...
- Prompts should use REDUX wrapped reversed tokens such as
  `{020000} + {010000} = <ans>` or `{300000} < {400000} = <ans>`.
- The backend normalizes prompts to the `<do> <calc> ... = <ans>` calculation protocol before analysis.
