...
- Dual checkpoint runtime behavior lives in `eur_is/backend/runtime.py`: `absolute` checkpoints use TransformerLens and expose attention/network features; `digit_roles` checkpoints use native PyTorch and currently do not expose raw attention/network features.
...
1. Start backend with an `absolute` checkpoint and use the frontend to analyze `03000000 + 03000000 = <ans>`.
...
3. Repeat with a `digit_roles` checkpoint and confirm the zip still contains PNG assets, with placeholder PNGs and manifest unavailable-section notes for attention/network data.
